...
Receive runs from `CHAT_MSG_ADDON` into `Comms:OnMessage`, keyed by sender and HSC1 transmission ID. Parts may arrive out of order; a duplicate index replaces the prior part, but there is no completed-transfer replay cache. A changed total discards that transfer. Once all indexes exist, Comms concatenates them and emits `HS_COMMS_MESSAGE` only if the full result is within the serializer byte limit. Partial transfer state expires after 30 seconds of inactivity. The sender-name self check runs before logging or state allocation. `SyncManager:Receive` then deserializes and validates the semantic envelope, resolves sender identity through GuildStore, and applies the existing domain authority, freshness, authorization, and import path. Inbound HSC accepts up to 1,500 parts per transfer, and the map of concurrent incomplete transfer IDs has no explicit aggregate cap; the serializer limit is checked only after reassembly. These are existing HSC1 limitations and are not claims of a complete hostile-input memory bound.
