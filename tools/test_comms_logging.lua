...
local ok,transmissionId=Comms:Send("1234567890123456789","WHISPER","Beta-Realm",50,{domain="character",objectId="Player-A\031equipment",messageKind="PAYLOAD",version=14,correlationId="corr-sync"});assert(ok and#queued==7 and transmissionId:find("HSC1%-"),"multipart send queues seven AceComm transport frames")
...
Comms:OnMessage(Comms.prefix,"HSC1|HSC1-active|1|3|abc","GUILD","Beta-Realm");Comms:OnMessage(Comms.prefix,"HSC1|HSC1-lost|1|2|xyz","GUILD","Beta-Realm");assert(Comms.incoming["Beta-Realm\031HSC1-active"]and Comms.incoming["Beta-Realm\031HSC1-lost"]and#timers==2 and not timers[2].cancelled,"parallel incoming transfers remain separated by sender and transmission ID")
clock=2029;Comms:OnMessage(Comms.prefix,"HSC1|HSC1-active|2|3|def","GUILD","Beta-Realm");assert(Comms.incoming["Beta-Realm\031HSC1-active"].lastReceivedAt==2029,"active fragment receipt refreshes transfer activity")
...
HolyStorm.Tasks.definitions[cleanup.id].execute();assert(Comms.incoming["Beta-Realm\031HSC1-active"]and not Comms.incoming["Beta-Realm\031HSC1-lost"],"cleanup drops the chunk-lost transfer without deleting another active transfer")
...
local sawLost,sawActive=false,false;for _,entry in ipairs(logs)do if entry.message=="Incomplete transmission expired"then if entry.context.transmissionId=="HSC1-lost"then sawLost=entry.context.packetPart==1 and entry.context.packetTotal==2 elseif entry.context.transmissionId=="HSC1-active"then sawActive=entry.context.packetPart==2 and entry.context.packetTotal==3 end end end;assert(sawLost and sawActive,"chunk-loss diagnostics retain transmission identity and received/total chunk counts")
