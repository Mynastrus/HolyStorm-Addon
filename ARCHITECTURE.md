...
Core centrally loads LibDataBroker-1.1, LibDBIcon-1.0, LibSharedMedia-3.0, AceComm-3.0/ChatThrottleLib, AceCommQueue-1.0, and LibGuildRoster-1.0. `HolyStorm.Libraries` exposes loaded-library versions, shared-media access, the canonical `ADDON_ICON` texture path (`LIVE/Holy_Storm/Images/minimap_logo.png`), and the one canonical Holy Storm launcher. The LDB launcher uses this icon; LibDBIcon presents that same launcher on the minimap and stores visibility/position in the existing profile database. The UI frame uses the same path in the native ButtonFrame portrait slot. The PNG is a Retail-loadable, power-of-two texture; LibSharedMedia remains for its supported media categories and is not used as an icon registry. LibQTip-1.0 remains owned by the UI addon for structured/multi-column tooltips. `SyncTransport` embeds AceComm and then AceCommQueue on the Comms object; every outbound legacy HSC1 frame uses that shared throttled queue, while TaskManager continues to schedule domain Sync work and fragment cleanup. LibGuildRoster is loaded and version-reported, but is not used for peer resolution: its separate roster does not replace GuildStore's GUID keyed identity, online owner lookup, or sender-to-GUID mapping. GuildStore remains the sole Holy Storm guild model. DeltaSync is NOT integrated.
...
`Sync/Comms.lua` keeps the `HolyStormSync` prefix and HSC1 application framing, including 220-byte payload pieces, message IDs, bounded outbound fragments, and receive reassembly. Receive accepts at most 300 fragments/66,000 bytes per transfer, holds at most 64 incomplete transfers globally and 16 per sender, evicts the oldest incomplete transfer when a cap is reached, and expires fragments after 30 seconds. This is not a second AceComm fragmentation pass: Comms rejects a complete HSC1 frame above 255 bytes, so AceComm sends each as a single addon message. AceComm multipart remains available only for logical messages larger than 255 bytes. We retain HSC1 for deployed-client compatibility and its bounded, expiring reassembly; AceComm's own receiver spool has no size cap or expiry. See `SYNC_PROTOCOL_AUDIT.md` for the embedded-source audit and tradeoffs.

`Sync/SyncTransport.lua` is the sole outbound adapter (`Send`, `SendGuild`, `SendWhisper`, `Cancel`, `GetDiagnostics`) and passes each HSC1 frame through AceCommQueue. AceCommQueue supplies per-prefix/channel/target queueing, inter-message priority, throttle integration and refusal retries. It serializes AceComm multipart messages when those occur, but does not perform Sync response suppression or domain retries. The old Comms per-frame TaskManager queue and `.08s` delay are retired. `Cancel` reports unsupported because AceCommQueue has no per-message cancellation API; shutdown suppresses already queued sends. `SyncManager` registers domains with metadata, export, validation, authorization and import functions. Discovery, Offer, Fetch, Payload and passive healing remain TaskManager work.
