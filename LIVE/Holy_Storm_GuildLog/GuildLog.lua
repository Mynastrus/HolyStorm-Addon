...
local metadata = { id = "guildLog", name = "GuildLog", displayName = L["DISPLAY_NAME"], description = L["DESCRIPTION"], version = addonVersion, moduleType = "feature", category="feature", permissions = { "guild-roster-read" }, dependencies = { "core", "ui" }, ui = { page = "guildLog", navigation = true }, enabledByDefault = true }
...
HolyStorm:RegisterModule(metadata, function(GuildLog)
    ...
    function GuildLog:Add(eventType, message)
        HolyStorm.Data.GuildStore:AddLogEntry(eventType, message)
    end
    function GuildLog:Scan()
        ...
        if not database.snapshot then HolyStorm.Data.GuildStore:SetLogSnapshot(current); self:Render(); return end
        ...
        HolyStorm.Data.GuildStore:SetLogSnapshot(current); self:Render()
    end
    ...
    function GuildLog:OnEnable()
        HolyStorm.Commands:RegisterSubcommand("log",{help=L["COMMAND_HELP"],execute=function()if not HolyStorm.UI:ShowPage("guildLog")then print(L["COMMAND_UNAVAILABLE"])end end})
