...
if HolyStorm.PlayerData then HolyStorm.PlayerData:RegisterBlock("professions",{fields={"professions"},event="HS_PROFESSIONS_UPDATED",staleAfter=604800})end
local metadata={id="professions",name="Professions",displayName=L["DISPLAY_NAME"],description=L["DESCRIPTION"],version=addonVersion,moduleType="feature",category="feature",permissions={{id="professions-read",category="Core"}},dependencies={"core"},capabilities={"character.scan.additional"},ui={page="professions",navigation=true},data={block="professions",capability="character.scan.additional"},enabledByDefault=false}
HolyStorm:RegisterModule(metadata,function(Module)
    ...
    function Module:Snapshot()
        ...
        return HolyStorm.PlayerData:WriteOwnedBlock(guid,"professions",result,"blizzard")
    end
    ...
    function Module:InitializeUI()
        ...
        HolyStorm.UI:RegisterPage("professions",page,metadata.displayName,function()Module:RefreshPage()end,{"HS_PROFESSIONS_UPDATED"}); HolyStorm.UI:AddNavigation("professions",11,"Interface\\Icons\\Trade_BlackSmithing",metadata.displayName,metadata.description,function()HolyStorm.UI:ShowPage("professions")end)
    end
    function Module:OnInitialize() HolyStorm:RegisterCapability("Professions","character.scan.additional",function(module)return module:Snapshot()end); HolyStorm:RegisterUIExtension("professions",{id="professions.page",order=11,initialize=function()Module:InitializeUI()end}) end
    function Module:OnEnable() HolyStorm.Events:Register("SKILL_LINES_CHANGED","professions",function()HolyStorm.Tasks:Enqueue("professions.scan",function()Module:Snapshot()end,{priority=7,debounce=1})end);local context=self.loadContext;if context and context.reason=="event"then HolyStorm.Tasks:Enqueue("professions.scan",function()Module:Snapshot()end,{priority=7,debounce=0})end end
