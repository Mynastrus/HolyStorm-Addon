...
local Sync={version=addonVersion,protocol=3,domains={},requests={},activeRequests={},heard={},heardAt={},sequence=0,maxOffers=100,knownOnline={},knownVersions={},requestTimeout=60,presenceTimeout=300,cleanupTimer=nil,cleanupDue=nil,cleanupTaskId=nil,loginSessionId=nil,presencePublished=false,peerVersionReceived=false,outdatedNotified=false}
...
function Sync:RunPublish(task)
 ...
 local channel=m.channel or(domain.getChannel and domain.getChannel(meta,"publish"))or"GUILD";log("DEBUG","discovery","Publishing lightweight metadata",{domain=m.domain,objectId=m.objectId,version=meta.version,channel=channel,reason=m.reason});self:QueueEnvelope("ANNOUNCE",m.domain,{offers={meta},reason=m.reason},channel,nil,70);return true
end
function Sync:RequestObject(domainId,objectId,options)
 ...
 if target then local requestId=self:NewRequestId();log("DEBUG","request","Requesting payload from online owner",{domain=domainId,objectId=objectId,owner=owner,target=target,requestId=requestId});return self:QueueFetch(domainId,objectId,target,localMeta and localMeta.version or-1,options.reason or"ON_DEMAND",localMeta and localMeta.revisionID,requestId)end
...
function Sync:OnDiscover(domainId,request,sender,channel)
 ...
 local delay=.15+(math.random()*1.1);HolyStorm.Tasks:Queue("Sync.Offer",{mergeKey=request.requestId.."\031"..sender,delay=delay,priority=domain.priority or 85,triggerSource="DISCOVERY_RESPONSE",metadata={domain=domainId,request=request,requester=sender,responseChannel=channel}});return true
end
...
function Sync:RunSelect(task)
 ...
 if best then local localMeta=self.domains[request.domain].getMetadata(m.objectId);self:QueueFetch(request.domain,m.objectId,best.sender,localMeta and localMeta.version or-1,request.reason or"DISCOVERY",localMeta and localMeta.revisionID,m.requestId)end
...
function Sync:QueueFetch(domainId,objectId,target,knownVersion,reason,knownRevisionID,requestId)
 local domain=self.domains[domainId];return HolyStorm.Tasks:Queue("Sync.Fetch",{mergeKey=key(domainId,objectId),priority=domain and domain.priority or 75,triggerSource=reason or"FETCH",metadata={domain=domainId,objectId=objectId,target=target,knownVersion=knownVersion,knownRevisionID=knownRevisionID,reason=reason or"FETCH",requestId=requestId or self:NewRequestId()}})
end
function Sync:RunFetch(task)local m=task.metadata;log("DEBUG","request","Requesting selected payload",{domain=m.domain,objectId=m.objectId,target=m.target,knownVersion=m.knownVersion,reason=m.reason,requestId=m.requestId});self:QueueEnvelope("FETCH",m.domain,{objectId=m.objectId,knownVersion=m.knownVersion,knownRevisionID=m.knownRevisionID,reason=m.reason,requestId=m.requestId},"WHISPER",m.target,75);return true end
function Sync:OnFetch(domainId,data,sender)
 ...
 self:QueueEnvelope("PAYLOAD",domainId,{objectId=data.objectId,metadata=meta,payload=payload,reason="REQUEST_RESPONSE",requestId=data.requestId},"WHISPER",sender,domain.priority or 70);log("DEBUG","payload","Payload queued for selected requester",{domain=domainId,objectId=data.objectId,version=meta.version,target=sender,reason="REQUEST_RESPONSE",requestId=data.requestId});return true
end
...
function Sync:GetDiagnostics()return{requests=HolyStorm.Utils.TableCount(self.requests),activeRequests=HolyStorm.Utils.TableCount(self.activeRequests),heard=HolyStorm.Utils.TableCount(self.heard),knownOnline=HolyStorm.Utils.TableCount(self.knownOnline),domains=HolyStorm.Utils.TableCount(self.domains),cleanupScheduled=self.cleanupTimer~=nil or self.cleanupTaskId~=nil}end
