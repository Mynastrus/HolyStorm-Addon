...
function Comms:Send(payload,preferred,target,priority,diagnostics)
 ...
 for part=1,total do local chunk=payload:sub((part-1)*self.chunkSize+1,part*self.chunkSize);local packetDiagnostics=compact(base);for field,value in pairs(base)do packetDiagnostics[field]=value end;packetDiagnostics.packetPart=part;packetDiagnostics.bytes=#chunk;local packet={message=table.concat({self.protocol,id,part,total,chunk},"|"),channel=channel,target=resolved,diagnostics=packetDiagnostics};self.pendingPackets=self.pendingPackets+1;HolyStorm.Tasks:Queue("Comms.SendPacket",{priority=tonumber(priority)or 50,delay=self.pendingPackets*.08,cooldown=.08,triggerSource="SYNC_SEND",metadata={packet=packet,diagnostics=packetDiagnostics}})end;return true,id
end
