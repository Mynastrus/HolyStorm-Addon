...
local mythicColumns={{width=.43},{width=.16,align="RIGHT"},{width=.18,align="RIGHT"},{width=.23,align="RIGHT"}}
...
local function bestMythicRun(dungeon)
 if type(dungeon)~="table"then return nil end;if type(dungeon.bestRun)=="table"then return dungeon.bestRun end;local best;local function consider(entry,overTime)if type(entry)~="table"then return end;local score=tonumber(entry.dungeonScore or entry.score or entry.seasonScore);local level=tonumber(entry.level or entry.bestLevel);if level and level>0 and(not best or(score or 0)>(best.score or 0)or((score or 0)==(best.score or 0)and level>(best.level or 0)))then best={level=level,score=score or 0,durationSec=tonumber(entry.durationSec),overTime=overTime==true or entry.overTime==true}end end;consider(dungeon.bestInTime,false);consider(dungeon.bestOverTime,true);for _,entry in pairs(type(dungeon.affixScores)=="table"and dungeon.affixScores or{})do consider(entry,type(entry)=="table"and entry.overTime)end;return best
end
...
local function mythic(view,context,definition)
 if blocked(view,definition)then return end;local snapshot,meta=C:GetSnapshot(context.characterUUID,"mythicPlus");local status=statusFor(context.characterUUID,"mythicPlus");if type(snapshot)~="table"then C:SetView(view,L[definition.labelKey],{L["NO_MYTHICPLUS"]},status);return end
 local dungeons={};if type(snapshot.dungeons)=="table"then for _,dungeon in pairs(snapshot.dungeons)do if type(dungeon)=="table"then dungeons[#dungeons+1]=dungeon end end end;if snapshot.seasonId==nil and snapshot.overallScore==nil and#dungeons==0 then C:SetView(view,L[definition.labelKey],{L["NO_MYTHICPLUS"]},status,meta);return end
 table.sort(dungeons,function(a,b)return tostring(a.name or"")<tostring(b.name or"")end);local lines={string.format(L["SEASON"],value(snapshot.seasonId)).."  |  "..L["RATING"]..": "..number(snapshot.overallScore)};if#dungeons>0 then lines[#lines+1]=tableRow({L["COLUMN_DUNGEON"],L["COLUMN_BEST_KEY"],L["COLUMN_DUNGEON_SCORE"],L["COLUMN_RUN"]},mythicColumns,"header",1320)end
 local completed,bestLevel,bestScore=0,nil,nil;for _,d in ipairs(dungeons)do local best=bestMythicRun(d);local level=best and tonumber(best.level);if best and(level or tonumber(best.score)or tonumber(best.durationSec))then completed=completed+1;if level and(not bestLevel or level>bestLevel)then bestLevel=level end else best=nil end;local dungeonScore=tonumber(d.score);if dungeonScore and(not bestScore or dungeonScore>bestScore)then bestScore=dungeonScore end;local texture=(type(d.texture)=="string"or type(d.texture)=="number")and d.texture;local icon=texture and hyperlink("teleport",d.challengeMapId or 0,"|T"..texture..":18:18|t")or"";local dungeonName=(type(d.name)=="string"or type(d.name)=="number")and tostring(d.name)or L["UNKNOWN"];local name=(type(d.instanceId)=="string"or type(d.instanceId)=="number")and hyperlink("journal",d.instanceId,dungeonName)or dungeonName;lines[#lines+1]=tableRow({(icon~=""and(icon.." ")or"")..name,value(level),number(dungeonScore),runResult(best)},mythicColumns,nil,1320)end
