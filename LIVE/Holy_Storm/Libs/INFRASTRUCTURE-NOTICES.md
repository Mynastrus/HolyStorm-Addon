...
These sources are loaded by the `Holy_Storm` core addon. Library behavior is upstream; only trailing whitespace was normalized in the Ace3 `ChatThrottleLib.lua`/license copy so `git diff --check` stays clean.
...
| LibDBIcon-1.0 | [CurseForge project](https://www.curseforge.com/wow/addons/libdbicon-1-0), v12.0.3, library minor 56 | CurseForge project metadata lists Ace3 Style BSD; the packaged TOC says All Rights Reserved, so the upstream licensing metadata conflicts. Depends on LibDataBroker-1.1, LibStub, and CallbackHandler. Its Retail package lists interface 120100. |
| LibSharedMedia-3.0 | [CurseForge project](https://www.curseforge.com/wow/addons/libsharedmedia-3-0), v12.1.0, library minor 12000002 | Upstream Lua header says LGPL-2.1 and `COPYING.txt` is included; CurseForge project metadata says All Rights Reserved, so upstream licensing metadata conflicts. Depends on LibStub and CallbackHandler. Current Retail package lists interface 120100. |
