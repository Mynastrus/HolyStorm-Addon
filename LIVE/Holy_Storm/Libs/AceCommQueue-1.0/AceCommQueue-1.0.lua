...
local MAJOR, MINOR = "AceCommQueue-1.0", 7
...
local function checkStall(q, key)
	...
	-- Decide first, report second. releaseStuck has two exits — re-queued for
	-- another attempt, or abandoned because the retry budget is spent (which
	-- includes a host that set retries to 0) — and the report must state the one
	-- that actually happened: a maintainer who reads "nothing is dropped" stops
	-- looking for the dropped message. Both counters are read BEFORE the call,
	-- because releaseStuck bumps q.stalls and its abandon path drains the queue.
	local firstStall = (q.stalls or 0) == 0
	local waiting    = #q.ALERT + #q.NORMAL + #q.BULK
	local resent     = releaseStuck(q, key, since)
	...
	if firstStall then
		geterrorhandler()(string.format(
			...
			.. "SendCommMessage wrapper that suppressed a send without calling "
			.. "callbackFn(callbackArg, 0, 0, nil) to release the queue. %s; further "
			.. "stalls here are counted in /acq status, not repeated.",
			MAJOR, since, tostring(key), waiting,
			resent and "The message is being re-sent and the queue released, so nothing "
				.. "is dropped and nothing stays blocked"
			or "The retry budget for this message was already spent, so it has been "
				.. "DROPPED (its callback reported reason=\"lost\") and the queue released"))
	else
		dbg("drain(%s): stalled again after %.0fs — stall #%d on this queue",
			key, since, q.stalls or 0)
	end
end
