...
function Workflows:GetDiagnostics()local active=0;for _,w in pairs(self.workflows)do if w.status~="COMPLETED"and w.status~="FAILED"and w.status~="CANCELLED"then active=active+1 end end;return{active=active,history=#self.history,historyLimit=self.maxHistory}end
