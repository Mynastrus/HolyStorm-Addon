...
local Commands={version=addonVersion}
...
function Commands:Initialize()
    ...
    local commandActions={open="",status="",help="?",options="options",log="log"}
...
function Commands:Execute(input)
    ...
    if value=="?" then printMessage(L["COMMAND_HELP_TITLE"]);printMessage(L["COMMAND_HELP_OPEN"]);printMessage(L["COMMAND_HELP_OPTIONS"]);printMessage(L["COMMAND_HELP_LOG"]);printMessage(L["COMMAND_HELP_GUIDE"]);printMessage(L["COMMAND_HELP_HELP"])
    elseif value=="" then HolyStorm.UI:Open()
    elseif value=="options" or value=="o" then local module=HolyStorm:GetModule("Options",true);if module then module:Open()end
    elseif value=="log" then if not HolyStorm.UI:ShowPage("guildLog")then printMessage(L["COMMAND_LOG_UNAVAILABLE"])end
    elseif value:match("^guide%s+")then local query=value:match("^guide%s+(.+)$");local share=query:match("^share%s+(.+)$");if share then query=share end;local guide,matches,reason=HolyStorm.Content:ResolveGuide(query);if guide then if share then local inserted,link=HolyStorm.RichLinks:InsertChatLink("guide",guide.id,string.format(L["GUIDE_LINK_LABEL"],guide.title));if not inserted then printMessage(link)end else HolyStorm.Content:Open(guide.id)end elseif reason=="AMBIGUOUS"then printMessage(L["GUIDE_AMBIGUOUS"]);for _,entry in ipairs(matches)do printMessage(HolyStorm.RichLinks:MakeHyperlink("guide",entry.id,entry.title))end else printMessage(L["GUIDE_NOT_FOUND"])end
    else printMessage(string.format(L["CORE_STATUS"],L[HolyStorm.Database:Get("enabled","profile")and"STATUS_ENABLED"or"STATUS_DISABLED"]))end
end
