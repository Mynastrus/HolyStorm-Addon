...
function Map:OpenContext(owner,entry)
 if HolyStorm.CharacterActions then return HolyStorm.CharacterActions:CreateContextMenu(owner,entry.characterUUID,{{text=L["ACTION_CREATE_POI"],enabled=true,callback=function()Map:OpenAsPOI(entry)end}})end
 return false
end
function Map:HandleClick(owner,button)if not owner.entry then return end;if button=="RightButton"then return self:OpenContext(owner,owner.entry)elseif HolyStorm.CharacterActions then return HolyStorm.CharacterActions:Open(owner.entry.characterUUID)end;return false end
